...
<table class="table table-striped">
	...
	<tbody>
		<tr>
			<td>PHP 5.3+</td>
			<td>必须</td>
			<td><?php echo version_compare(PHP_VERSION, '5.3.0', '>')?'<font color="green">'.PHP_VERSION.'</font>':'<font color="red">'.PHP_VERSION.'</font>'; ?></td>
			<td>PHP版本支持</td>
		</tr>
		<tr>
			<td>curl_exec()</td>
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过凌吾自助下单系统</div>
