...
case 'pay':
	...
	if($tool && $tool['active']==1){
		...
		if($conf['forcermb']==1 && !$islogin2)exit('{"code":4,"msg":"你还未登录"}');
		...
		if($need==0 && $tid!=$_SESSION['gift_tid']){
			$thtime=date("Y-m-d").' 00:00:00';
			if($_SESSION['blockfree']==true || $DB->count("SELECT count(*) FROM `shua_pay` WHERE `tid`='{$tid}' and `money`=0 and `ip`='$clientip' and `status`=1 and `addtime`>'$thtime'")>=1){
...
case 'cancel':
	...
	if($srow['status']==0){
		$DB->query("DELETE FROM shua_pay WHERE trade_no='{$orderid}'");
...
case 'order': //订单进度查询
	...
	if($tool['is_curl']==4 || $row['djzt']==3){
		...
		if($shequ['type']==1){
			...
			$list = jiuliu_chadan($shequ['username'], $shequ['password'], $row['djorder'], $tool['goods_id']);
	...
	$result=array('code'=>0,'msg'=>'succ','name'=>$tool['name'],'money'=>$row['money'],'date'=>$row['addtime'],'inputs'=>showInputs($row,$input,$inputs),'list'=>$list,'kminfo'=>$kmdata,'alert'=>$tool['alert'],'desc'=>$tool['desc'],'status'=>$row['status'],'result'=>$row['result'],'complain'=>intval($conf['show_complain']),'islogin'=>$islogin2);
	exit(json_encode($result));
	break;
case 'changepwd':
