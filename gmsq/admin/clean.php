...
$DB->query("DELETE FROM `shua_qiandao` WHERE time<'".date("Y-m-d H:i:s",strtotime("-30 days"))."'");
$DB->query("OPTIMIZE TABLE `shua_qiandao`");
showmsg('删除30天前签到记录成功！',1);
}elseif($mod=='cleanwork'){
$DB->query("DELETE FROM `shua_workorder` WHERE addtime<'".date("Y-m-d H:i:s",strtotime("-30 days"))."'");
$DB->query("OPTIMIZE TABLE `shua_workorder`");
