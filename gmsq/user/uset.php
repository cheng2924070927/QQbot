...
<?php
	...
	$sitename=daddslashes(trim(strip_tags($_POST['sitename'])));
	$title=daddslashes(trim(strip_tags($_POST['title'])));
	$keywords=daddslashes(trim(strip_tags($_POST['keywords'])));
	$description=daddslashes(trim(strip_tags($_POST['description'])));
	...
	if($sitename==null){
		...
		if($userrow['power']==2){
			if(!is_numeric($ktfz_price) || !preg_match('/^[0-9.]+$/', $ktfz_price))exit("<script language='javascript'>alert('普及分站价格输入不规范');history.go(-1);</script>");
			if(!is_numeric($ktfz_price2) || !preg_match('/^[0-9.]+$/', $ktfz_price2))exit("<script language='javascript'>alert('专业分站价格输入不规范');history.go(-1);</script>");
			...
			$sds=$DB->query("update shua_site set sitename='$sitename',title='$title',keywords='$keywords',description='$description',anounce='$anounce',modal='$modal',bottom='$bottom',alert='$alert',template='$template' where zid='{$userrow['zid']}'");
		}
...
<div class="panel-body">
  <form action="./uset.php?mod=site_n" method="post" class="form-horizontal" role="form">
	...
	<div class="form-group">
	  ...
	  <div class="col-sm-10"><div class="input-group"><input type="text" name="domain" value="<?php echo $userrow['domain']; ?>" class="form-control" disabled/><div class="input-group-addon"><a href="cdomain.php">自助更换域名</a></div></div></div>
	</div><br/>
	<?php }?>
	<div class="form-group">
	  <div class="col-sm-offset-2 col-sm-10"><input type="submit" name="submit" value="修改" class="btn btn-primary form-control"/><br/>
...
<?php
	...
	if(array_key_exists('sitename',$arr)){
		if(in_array('anounce',$content))$anounce = addslashes(str_replace($arr['kfqq'],$userrow['qq'],$arr['anounce']));
		else $anounce = addslashes($userrow['anounce']);
		if(in_array('modal',$content))$modal = addslashes(str_replace($arr['kfqq'],$userrow['qq'],$arr['modal']));
		else $modal = addslashes($userrow['modal']);
		if(in_array('bottom',$content))$bottom = addslashes(str_replace($arr['kfqq'],$userrow['qq'],$arr['bottom']));
		else $bottom = addslashes($userrow['bottom']);
...
echo '<div class="panel panel-success"><div class="panel-heading"><h3 class="panel-title">更改首页LOGO</h3></div><div class="panel-body">提示：部分模板不显示logo图片，是正常现象！<br/>';
if($_POST['s']==1){
