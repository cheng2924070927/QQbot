...
?>
        <table class="table table-striped" style="font-size:12px" >
          <thead ><tr ><th style="font-size:14px">名称</th><th style="font-size:14px">成本</th><?php if($userrow['power']==2){?><th style="font-size:14px">下级</th><?php }?><th style="font-size:14px">销售</th><th style="font-size:10px"><span class="glyphicon glyphicon-eye-open"></span></th><th style="font-size:14px">操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
echo '<tr><td><b><a title="点此下单" style="color:#000" href="../?cid='.$res['cid'].'&tid='.$res['tid'].'">'.$res['name'].'</a></td><td><font color="#7D9EC0">'.($userrow['power']==2?$price_obj->getToolCost2($res['tid']).'元</font></td><td><font color="#9400D3">'.$price_obj->getToolCost($res['tid']):$price_obj->getToolCost($res['tid'])).'元</font></td><td><font color="#FF0000">'.$price_obj->getToolPrice($res['tid']).'元</font> </td><td>'.($price_obj->getToolDel($res['tid'])==1?'<font color=red><span class="glyphicon glyphicon-remove"></span></font>':'<font color=green><span class="glyphicon glyphicon-ok"></span></font>').'</td><td><a href="./shoplist.php?my=edit&tid='.$res['tid'].'" class="label label-primary">编辑</a></td></tr>';
}
?>
          </tbody>
        </table>
