...
{
$zid=intval($_GET['zid']);
$row=$DB->get_row("select * from shua_site where zid='$zid' and upzid='{$userrow['zid']}' and power=1 limit 1");
if(!$row)
	showmsg('当前记录不存在！',3);
echo '<div class="panel panel-primary">
...
$sql="insert into `shua_site` (`power`,`upzid`,`domain`,`domain2`,`user`,`pwd`,`rmb`,`qq`,`sitename`,`keywords`,`description`,`anounce`,`alert`,`addtime`,`endtime`,`status`) values ('1','".$userrow['zid']."','".$domain."',NULL,'".$user."','".$pwd."','0','".$qq."','".$sitename."','".$keywords."','".$description."','".$anounce."','".$alert."','".$date."','".$endtime."','1')";
if($DB->query($sql)){
...
{
$zid=intval($_GET['zid']);
$rows=$DB->get_row("select * from shua_site where zid='$zid' and upzid='{$userrow['zid']}' and power=1 limit 1");
if(!$rows)
	showmsg('当前记录不存在！',3);
...
if($DB->query("update shua_site set domain2='$domain2',qq='$qq',sitename='$sitename',endtime='$endtime' where zid='{$zid}'"))
...
$numrows=$DB->count("SELECT count(*) from shua_site where upzid='{$userrow['zid']}' and power=1");
if(isset($_GET['zid'])){
	$zid=intval($_GET['zid']);
