...
{
$id=intval($_GET['id']);
$row=$DB->get_row("select * from auth_qqs where id='$id' limit 1");
echo '<div class="panel panel-primary">
...
if($DB->query("insert into `auth_qqs` (`qq`,`oqq`,`date`,`active`) values ('".$qq."','".$oqq."','".$date."','".$active."')")){
	$city=get_ip_city($clientip);
	$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','新增授权','".$date."','".$city."','".$qq."')");
...
if($DB->query("update auth_qqs set qq='$qq',oqq='$oqq',active='$active' where id='{$id}'"))
...
if(!$rows)
	showmsg('当前记录不存在！',3);
$sql="DELETE FROM auth_qqs WHERE id='$id'";
if($DB->query($sql)){
	$city=get_ip_city($clientip);
...
if(isset($_GET['kw'])) {
	...
	$sql=" uid='$uid'";
	$numrows=$DB->count("SELECT count(*) from auth_qqs WHERE{$sql}");
	$con='代理UID:'.$uid.' 共有 <b>'.$numrows.'</b> 条记录';
	$link='&uid='.$uid;
}else{
	$sql=" 1";
	$numrows=$DB->count("SELECT count(*) from auth_qqs where{$sql}");
	$con='系统共有 <b>'.$numrows.'</b> 个已授权用户';
}
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>ID</th><th>机器人QQ</th><th>主人QQ</th><th>添加时间</th>><th>代理ID</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
